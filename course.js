...
// ===================== PART G — FROM BASE STOCK TO COMPLIANT COOLANT =====================
{ch:"G · Base stock → coolant", title:"Start: the Group III spec sheet",
 body:`<p>Now build the fluid forward — from a lube base-stock spec sheet to an OCP-compliant coolant, number by number. Here is a representative light Group III grade (4 cSt @100 °C class) exactly as it leaves the hydrocracker/dewaxer, made for <i>lubricants</i>:</p>
 <div class="eqn" style="font-size:12.5px;text-align:left;line-height:1.8">
 Saturates 99.5 % · Sulfur 8 ppm · VI 125<br>
 KV@40 °C 19.5 cSt · KV@100 °C 4.2 cSt · density 0.815 g/mL<br>
 Flash 220 °C · Pour −24 °C<br>
 Thermal cond. 0.130 W/m·K · Specific heat 2.00 kJ/kg·K<br>
 Dielectric strength 14 kV/mm · Resistivity 1×10¹⁴ Ω·cm</div>
 <p>Dynamic viscosity @40 °C = KV × density = 19.5 × 0.815 = <b>15.9 cP</b>. Nothing has been added yet — this is an ordinary lube base stock.</p>
 <div class="defends">🛡️ The thesis in one move: this spec sheet is the <i>starting</i> material. The next lessons show every number that already clears OCP, and the few that need work.</div>`},

{ch:"G · Base stock → coolant", title:"Map it onto the OCP envelope",
 body:`<p>Lay the untouched base stock against the OCP immersion-fluid limits:</p>
 <div class="eqn" style="font-size:12px;text-align:left;line-height:1.7">
 Saturates 99.5 ≥ 90 % ✓ · Sulfur 8 ≤ 300 ppm ✓<br>
 Specific heat 2.00 ≥ 1.8 ✓ · Flash 220 ≥ 150 °C ✓<br>
 Dielectric str. 14 ≥ 6 kV/mm ✓ · Resistivity 1e14 ≥ 2e11 Ω·cm ✓<br>
 Pour −24 ≤ −20 °C ✓ · Kinematic visc. 19.5 ≤ 20.5 cSt ✓<br>
 <b>Dynamic visc. 15.9 ≤ 15 cP ✗ GAP</b><br>
 <b>Thermal cond. 0.130 ≥ 0.14 W/m·K ✗ GAP</b></div>
 <p><b>8 of 10 lines pass untouched.</b> Only two need engineering — dynamic viscosity and bulk thermal conductivity — and neither needs a new refinery unit.</p>
 <div class="defends">🛡️ You can now answer: <i>"How far is a lube base stock from a coolant spec?"</i> — 80 % of the way there on day one; the gap is two properties, both closable by formulation.</div>`,
 check:{q:"How many of the 10 OCP thermophysical lines does the raw base stock already meet?",
  opts:[{t:"8 of 10 — only dynamic viscosity and thermal conductivity fall short",ok:true},{t:"Almost none — it needs heavy reformulation",ok:false}],
  reveal:"8 of 10. The dielectric, thermal-capacity, flash, pour and one viscosity line all pass as-produced; only dynamic viscosity (15.9 cP) and bulk k (0.130) are gaps."}},

{ch:"G · Base stock → coolant", title:"Gap A — closing viscosity",
 body:`<p>Dynamic viscosity 15.9 cP must drop below 15. Two independent levers, computed with the Walther / ASTM D341 viscosity–temperature law:</p>
 <div class="eqn"><span style="font-size:.8em">loglog</span>(ν + 0.7) = <i>A</i> − <i>B</i> · log₁₀<i>T</i></div>
 <p>• <b>Lever 1 — lighter grade</b> (3 cSt): KV@40 °C = 13.9 cSt → 13.9 × 0.810 = <b>11.3 cP</b> ✓ (and 13.9 &lt; 20.5 cSt ✓).<br>
 • <b>Lever 2 — warm operation</b> (keep the 4 cSt grade, run at the normal tank temperature): @45 °C → 13.3 cP, @50 °C → 11.2 cP ✓.</p>
 <p>Either lever clears it. Grade selection is a purchasing choice; warm operation is free.</p>
 <div class="defends">🛡️ You can now answer: <i>"Isn't a lube base oil too viscous to pump around a server?"</i> — A light grade is 11 cP at 40 °C, and warm operation drops it further; both are under the 15 cP OCP limit.</div>`},

{ch:"G · Base stock → coolant", title:"Gap B — closing thermal conductivity",
 body:`<p>Bulk conductivity 0.130 must reach 0.14 W/m·K. Two routes:</p>
 <p>• <b>System route (preferred)</b> — bulk k is not actually the OCP <i>gate</i>; the junction-temperature spec is. With the convective design, Tⱼ = 40 + 700 × (0.0150 + 0.0100 + 0.002143) = <b>59 °C ≪ 88 °C</b>. The thermal requirement is met at k = 0.130, no additive needed.<br>
 • <b>Fluid route (optional)</b> — a nanofluid raises effective k (next lessons), at a dielectric cost.</p>
 <div class="eqn"><i>R</i><sub>conv</sub> = 1/(<i>h A</i>) dominates — so <i>h·A</i>, not raw k, carries the heat</div>
 <div class="defends">🛡️ You can now answer: <i>"Its conductivity is below spec — how can it cool?"</i> — Because immersion cooling is convection-limited: the junction lands at 59 °C via h·A even at k = 0.130, so the thermal spec is met without touching the fluid.</div>`,
 check:{q:"Why can the thermal spec pass even though bulk k (0.130) is under the 0.14 target?",
  opts:[{t:"The real gate is junction temp, and convection (h·A) holds Tj at 59 °C ≪ 88 °C",ok:true},{t:"The 0.14 W/m·K limit is not a real requirement",ok:false}],
  reveal:"Convection dominates. R_conv = 1/(hA) sets the junction temperature; at k = 0.130 the design still lands Tj = 59 °C, well under the 88 °C ceiling."}},

{ch:"G · Base stock → coolant", title:"Processes: drying & filtration",
 body:`<p>Before any chemical goes in, two physical <b>processes</b> do the dielectric heavy lifting — because dissolved water and particulates, not the oil itself, limit breakdown voltage:</p>
 <div class="eqn" style="font-size:13px;text-align:left;line-height:1.85">
 <b>P1 · Vacuum dehydration + degassing</b> (no chemical)<br>
 &nbsp;&nbsp; water 45 → 8 ppm · BDV 35 → 52 kV (+49 %) · ρ 1×10¹⁴ → 1.2×10¹⁵ Ω·cm<br>
 <b>P2 · Fine filtration (5 µm)</b> (no chemical)<br>
 &nbsp;&nbsp; BDV 52 → 55 kV · ρ → 1.5×10¹⁵ Ω·cm</div>
 <p>Drying alone buys most of the breakdown-voltage headroom — water is the number-one BDV killer. After P2 the fluid sits at the <b>55 kV / 1.5×10¹⁵ Ω·cm</b> baseline the compliance model uses.</p>
 <div class="defends">🛡️ You can now answer: <i>"Where does the 55 kV BDV come from?"</i> — From drying (35→52) and filtration (52→55). No additive raises it; removing water and particles does.</div>`},

{ch:"G · Base stock → coolant", title:"The additive package",
 body:`<p>Now the chemistry — deliberately tiny and <b>ashless</b>, because conductive/metallic additives (like ZDDP) wreck the dielectric. Total load ≈ <b>0.33 wt%</b>:</p>
 <div class="eqn" style="font-size:12.5px;text-align:left;line-height:1.85">
 <b>A1 · Antioxidant</b> — ashless hindered-phenol + aminic, <b>0.30 wt%</b><br>
 &nbsp;&nbsp; RPVOT 25 → 300 min (×12) → enables ≥ 5 yr life<br>
 &nbsp;&nbsp; tan δ 1.0 → 1.4×10⁻⁴ (still 350× under limit) · Dk/BDV/ρ unchanged<br>
 <b>A2 · Metal deactivator</b> — tolyltriazole deriv., <b>0.03 wt%</b><br>
 &nbsp;&nbsp; Cu strip 1b → 1a · kills Cu-catalysed oxidation · tan δ +0.1×10⁻⁴</div>
 <p>The antioxidant buys the lifetime; the metal deactivator protects it against hot copper busbars. Both are non-metallic, so the dielectric numbers barely move. (A pour-point depressant is available but skipped — the light grade already passes at −24 °C.)</p>
 <div class="defends">🛡️ You can now answer: <i>"What's actually in the fluid?"</i> — ~0.33 wt% of ashless antioxidant + metal deactivator. No ZDDP, no detergents, no ash — nothing that raises conductivity.</div>`,
 check:{q:"Why is the additive package deliberately ashless and metal-free?",
  opts:[{t:"Metallic/conductive additives (e.g. ZDDP) would degrade resistivity and BDV",ok:true},{t:"Ashless additives are cheaper than metallic ones",ok:false}],
  reveal:"Dielectric protection. ZDDP and metallic detergents raise conductivity and leave ash — fatal for a coolant — so only ashless antioxidant + metal deactivator are used, ~0.33 wt% total."}},

{ch:"G · Base stock → coolant", title:"The nanoparticle trade-off",
 body:`<p>The one additive that <b>costs</b> you: nanoparticles for conductivity. Computed with Maxwell (for k) and Maxwell–Garnett (for Dk), at 3 vol% Al₂O₃ + 0.5 wt% dispersant:</p>
 <div class="eqn" style="font-size:13px;text-align:left;line-height:1.85">
 k 0.130 → <b>0.142</b> W/m·K &nbsp; ✓ gain (clears ≥ 0.14)<br>
 Dk 2.150 → <b>2.251</b> &nbsp; ⚠ cost (eats margin toward 2.30)<br>
 tan δ 1.5 → <b>8×10⁻⁴</b> &nbsp; ⚠ cost (interfacial loss, still &lt; 0.05)<br>
 BDV 55 → <b>52 kV</b> &nbsp; ⚠ −5 % if not surface-functionalised</div>
 <p>So spending an additive to win 0.012 W/m·K of conductivity <i>costs</i> ~0.10 of Dk margin and some BDV. That's why it stays <b>optional</b> — the system route already meets the thermal spec at k = 0.130 without paying this price.</p>
 <div class="defends">🛡️ You can now answer: <i>"Why not just add nanoparticles for conductivity?"</i> — Because the numbers show it eats dielectric margin (Dk 2.15→2.25, BDV −5 %). Convective design meets the thermal spec without it, so it is held in reserve.</div>`,
 check:{q:"Adding 3 vol% Al₂O₃ raises k to 0.142. What does it cost?",
  opts:[{t:"Dielectric margin — Dk rises to 2.25 and BDV drops ~5 %",ok:true},{t:"Nothing — it improves every property at once",ok:false}],
  reveal:"Dielectric margin. Maxwell-Garnett gives Dk 2.15→2.25 (toward the 2.30 limit) and BDV falls ~5 %. A real trade-off — hence optional."}},

{ch:"G · Base stock → coolant", title:"Spec sheet → compliant coolant",
 body:`<p>The final formulated fluid (system route, no nanoparticles) against every OCP line:</p>
 <div class="eqn" style="font-size:12px;text-align:left;line-height:1.7">
 Dynamic visc. 11.3 ≤ 15 cP ✓ · Kinematic 13.9 ≤ 20.5 cSt ✓<br>
 Thermal cond. 0.130 → met via Tⱼ = 59 °C ✓ · BDV 55 ≥ 45 kV ✓<br>
 Dk 2.15 ≤ 2.30 ✓ · tan δ 1.5×10⁻⁴ ≤ 0.05 ✓<br>
 Resistivity 1.5×10¹⁵ ≥ 2×10¹¹ Ω·cm ✓ · Pour −24 ≤ −20 °C ✓</div>
 <p>Feed it through the coupled thermal/electrical/lifetime models and the six compliance specs land at <b>99.70 % joint</b> (vs the 85 % bar), with Tⱼ = 59.3 °C and P5 life = 5.8 yr.</p>
 <div class="eqn" style="font-size:14px">base stock (8/10) → +2 fixes → +0.33 wt% ashless pkg → <b>99.70 % OCP-compliant</b></div>
 <div class="defends">🛡️ You can now trace the entire path: lube spec sheet → 8/10 free → grade/warm-op closes viscosity → convective design meets thermal → drying+filtration set the dielectrics → 0.33 wt% ashless package secures life → all six OCP specs pass at 99.70 %.</div>`},

{ch:"G · Base stock → coolant", title:"Defending the dossier",
 ...
 • <b>Build path</b> — a lube base stock meets 8/10 OCP lines untouched; grade choice + convective design close viscosity and thermal; drying + filtration + ~0.33 wt% ashless additives finish the job — no new refinery unit.<br>
